...
The structure actions appear above the results list and use the extension's selected language.
Structure synchronization is available in the full panel only; compact mode stays focused on quick import and consolidation.
...
- A structure synchronization takes priority over an automatic background scan. If files are actively being imported, wait for that import to finish first.
